...
public class GameController : MonoBehaviour {
    ...
    {
        Debug.Log("Saving calibration data");
...
class CalibrationData
{
    ...
    public Vector3 rightControllerOffset {
        get { return new Vector3(data[0 * 3 + 1], data[0 * 3 + 1], data[0 * 3 + 2]);}
        set { data[0 * 3 + 1] = value.x; data[0 * 3 + 1] = value.y; data[0 * 3 + 2] = value.z;}
    }
    public Vector3 lowerLeftScreenCorner
    {
        get { return new Vector3(data[1 * 3 + 1], data[1 * 3 + 1], data[1 * 3 + 2]); }
        set { data[1 * 3 + 1] = value.x; data[1 * 3 + 1] = value.y; data[1 * 3 + 2] = value.z; }
    }
    public Vector3 upperLeftScreenCorner
    {
        get { return new Vector3(data[2 * 3 + 1], data[2 * 3 + 1], data[2 * 3 + 2]); }
        set { data[2 * 3 + 1] = value.x; data[2 * 3 + 1] = value.y; data[2 * 3 + 2] = value.z; }
    }
    public Vector3 upperRightScreenCorner
    {
        get { return new Vector3(data[3 * 3 + 1], data[3 * 3 + 1], data[3 * 3 + 2]); }
        set { data[3 * 3 + 1] = value.x; data[3 * 3 + 1] = value.y; data[3 * 3 + 2] = value.z; }
    }
